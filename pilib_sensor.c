...
/* pi_sens_acs713 -- ACS713 current sensor (20A)
 * @reading -- "raw" reading in range [0,1)
 * @vref -- optional reference voltage -- default 5.00 (not used)
 */
int pi_sens_acs713(lua_State * L)
{
   lua_Number  reading ;
   lua_Number  vref = 5.0 ;

   reading = luaL_checknumber( L, 1 );
//   vref = luaL_optnumber( L, 1, 5.0 );
 ...
 * @vref -- optional reference voltage -- default 5.00 (not used)
 */
int pi_sens_acs723(lua_State * L)
{
   lua_Number  reading ;
   lua_Number  vref = 5.0 ;
 ...
 * The selected resistors are: (all in kOhms)
 ...
 * A "raw" Reading is [0,1) referenced to Vcc
 *   Sensor = Vcc * Reading
 * Solving for I gives
